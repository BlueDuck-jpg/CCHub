...
local window = library.CreateLib("CCHub v1 - Test your time!, Welcome " .. check_name, GrapeTheme)
...
Main:NewButton("Get Best Time (Main Lobby)", "Get Best Time (Main Lobby)", function()
    local BestTime = game:GetService("Workspace").Leaderboard1["LeaderboardPart "].SurfaceGui.ScrollingFrame.A1.PlrName.Text:split(": ")[2]
    local ohString1 = tostring(BestTime)
    local ohString2 = "the Main obby"
    
    game:GetService("ReplicatedStorage").ApplyTime:FireServer(ohString1, ohString2)
end)
...
Credits:NewDropdown("Scripters", "List of scripters", {"Blue Duck#8344", "Feliposo21#3197"}, function(t)
    setclipboard(t) 
end)Credits:NewDropdown("Scripters", "List of scripters", {"Blue Duck#8344", "Feliposo21#3197"}, function(t)
    setclipboard(t) 
end)
